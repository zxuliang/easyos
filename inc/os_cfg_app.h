...
#define CPU_STK_GROWTH_HI_TO_LO		0xABCDABCD
...
#define CPU_INT_EN()	do { irq_enable(); } while(0)
#define CPU_INT_DIS()	do { irq_disable();} while(0)

#define CPU_SR_ALLOC() 		CPU_DATA cpu_sr = 0
#define OS_ENTER_CRITICAL()	cpu_sr = irq_lock_save()
#define OS_EXIT_CRITICAL() 	irq_unlock_restore(cpu_sr)

#define CPU_CRITICAL_ENTER()	cpu_sr = irq_lock_save()
#define CPU_CRITICAL_EXIT()	irq_unlock_restore(cpu_sr)
