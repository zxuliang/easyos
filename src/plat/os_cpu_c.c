...
CPU_STK  *OS_CPU_ExceptStkBase = 0;
CPU_STK  OS_CPU_ExceptStkPtr = 0;

static CPU_STK   OSCfg_ISRStk_ISR[OS_CFG_ISR_STK_SIZE];
void  OSInitHook (void)
{
    ...
    for (i = 0u; i < OSCfg_ISRStkSize; i++) {
        *p_stk++ = (CPU_STK)0u;
        OSCfg_ISRStk_ISR[i] = 0;
    }
    addr = (CPU_STK)(OSCfg_ISRStkBasePtr + OSCfg_ISRStkSize);
    addr &= ~3;
    OS_CPU_ExceptStkBase = (CPU_STK *)addr;
}
